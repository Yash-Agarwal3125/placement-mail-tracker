...
def test_date_only_deadline_is_all_day(mock_settings):
    ...
    opp = _opp(deadline="15 June 2027", priority="HIGH")
    events, anomalies = derive_events([opp], mock_settings)
    ...
    assert event.start_iso == "2027-06-15"
    assert event.end_iso == "2027-06-15"
...
def test_all_eligible_mode_includes_oa_interview_regardless_of_my_status(mock_settings):
    ...
    opp = _opp(
        ...
        oa_date="17-Jun-2027 05:30 PM",
        interview_date="20-Jun-2027 10:00 AM",
        my_status="NOT_APPLIED",
        priority="HIGH",
    )
...
def test_not_eligible_and_unknown_company_produce_zero_events(mock_settings):
    opp_not_eligible = _opp(
        id=1, deadline="15 June 2027", eligibility_status="NOT_ELIGIBLE_BRANCH"
    )
    opp_unknown_company = _opp(id=2, deadline="16 June 2027", company_name="Unknown")
...
def test_stopword_fragment_company_name_produces_zero_events(mock_settings):
    ...
    opp = _opp(id=3, deadline="16 June 2027", company_name="Is Scheduled On")
    events, anomalies = derive_events([opp], mock_settings)
    assert events == []
    assert len(anomalies) == 1
...
def test_missing_drive_kind_defaults_to_placement(mock_settings):
    ...
    opp = _opp(id=1, deadline="15 June 2027", priority="HIGH")
...
def test_collision_guard_drops_higher_opportunity_id(mock_settings):
    opp1 = _opp(id=1, company_name="Acme Corp", deadline="15 June 2027", priority="HIGH")
    opp2 = _opp(id=2, company_name="Acme Corp", deadline="15 June 2027", priority="HIGH")
    events, anomalies = derive_events([opp1, opp2], mock_settings)
...
class TestDeadlineGatedOnDemonstratedInterest:
    ...
    def test_high_priority_gets_deadline_event_despite_not_applied(self, mock_settings):
        opp = _opp(deadline="15 June 2027", my_status="NOT_APPLIED", priority="HIGH")
        events, _ = derive_events([opp], mock_settings)
        deadline_events = [e for e in events if e.event_type == "DEADLINE"]
        assert len(deadline_events) == 1
...
class TestMissedDeadlineExcludesTheWholeDrive:
    """Explicit user request, 2026-09-09: a NOT_APPLIED drive whose apply-by
    deadline has already passed is moot -- it "serves no purpose" and just
    confuses attendance tracing later, so no event of any type is admitted
    for it, not just the DEADLINE reminder."""

    def test_missed_deadline_excludes_every_event_type(self, mock_settings):
        opp = _opp(
            deadline="15 June 2020",
            oa_date="17-Jun-2020 05:30 PM",
            interview_date="20-Jun-2020 10:00 AM",
            ppt_date="18-Jun-2020 03:00 PM",
            my_status="NOT_APPLIED",
            priority="HIGH",
        )
        settings = mock_settings.model_copy(update={"calendar_sync_mode": "all_eligible"})
    ...
    def test_applied_survives_a_past_deadline(self, mock_settings):
        """Once you've actually applied, a past deadline is expected
        history, not a missed opportunity -- is_deadline_gated_out (not
        this gate) already governs the DEADLINE reminder itself."""
        opp = _opp(
            deadline="15 June 2020",
            oa_date="17-Jun-2020 05:30 PM",
            my_status="APPLIED",
    ...
    def test_future_deadline_not_yet_applied_is_unaffected(self, mock_settings):
        opp = _opp(deadline="15 June 2027", my_status="NOT_APPLIED", priority="HIGH")
        events, _ = derive_events([opp], mock_settings)
        assert any(e.event_type == "DEADLINE" for e in events)

    def test_unparseable_deadline_cannot_prove_a_miss(self, mock_settings):
        opp = _opp(
            deadline="Round 3 at 5 in Lab 2",
            oa_date="17-Jun-2027 05:30 PM",
            my_status="NOT_APPLIED",
        )
        settings = mock_settings.model_copy(update={"calendar_sync_mode": "all_eligible"})
